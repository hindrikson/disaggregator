...
def check_matching_indices(df, shape_df):
    ...
    #     logger.warning(
    #         f"Indices in first df but NOT in the second: {missing_in_shapefile}"
    #     )
    # if missing_in_df:
    #     logger.warning(f"Indices in second df but NOT in the first : {missing_in_df}")
...
def database_raw(query):
    ...
    if isinstance(json_data, dict) and "data" in json_data:
        # Response has metadata wrapper, extract data array
        data_array = json_data["data"]
        df = pd.DataFrame(data_array)
    elif isinstance(json_data, list):
        # Response is already a data array
...
def dict_region_code(
    keys="id_ags", values="natcode_nuts3", level="lk", raw=False, **kwargs
):
    """
    ...
    """
    logger.warning(
        "The mapping of the AGS to NUTS-3 regions is done using the file t_nuts3_lk.csv which is for a unknown year. This may not correspond entirely to the AGS number of the data fetched for another year from opendata."
    )
    dict_source = "local"
    # Argument error handling
    assert dict_source in [
        "local",
        "database",
    ...
    if dict_source == "local" and level == "lk":
        # NOTE: the file t_nuts3_lk.csv is the original file
        df = pd.read_csv(data_in("regional/t_nuts3_lk.csv"), encoding="utf-8")
        # NOTE: the file below nut3_lk_2024.csv is newer create with merge.py
        # df = pd.read_csv(data_in("regional/nuts3_lk_2024.csv"), encoding="utf-8")
...
def append_region_name(df):
    ...
    return df.assign(
        region_name=lambda x: x.index.map(
            dict_region_code(keys=keys, values="name", level=level, dict_source="local")
        )
    )
...
def gas_load_profile_parameters_dict():
    ...
    return {
        "A": {
            "BA": 0.277008711731108,
            "BD": 1.4633681573375,
            "BH": 0.987428301992787,
            "GA": 1.15820816823062,
            "GB": 1.82137779524266,
            "HA": 1.97247753750471,
            "KO": 1.35545152289308,
        ...
        "C": {
            ...
            "PD": 8.4,
            "WA": 4.866274683,
            "SpaceHeating-MFH": 5.8164304,
            "SpaceHeating-EFH": 5.6727847,
            "Cooking_HotWater-HKO": 6.5718175,
        },
        "D": {
            "BA": 0.4865118291885,
            "BD": 0.0808834761578303,
            "BH": 0.226571574644788,
            "GA": 0.223568019279065,
            "GB": 0.0678117914984112,
            "HA": 0.0345781570412447,
            "KO": 0.0990618615825365,
            "MF": 0.0873351930206002,
            "MK": 0.038531702714089,
            "PD": 0.0702545839208687,
            "WA": 0.491227957971774,
            "SpaceHeating-MFH": 0.0873352,
            "SpaceHeating-EFH": 0.0716431,
            "Cooking_HotWater-HKO": 0.7107710,
        },
        "mH": {
            "BA": -0.00948491309440127,
            "BD": -0.047579990370696,
            "BH": -0.0339019728779373,
            "GA": -0.0410334784248699,
            "GB": -0.0607665689685263,
            "HA": -0.0742174022298938,
            "KO": -0.0526486914295292,
            "MF": -0.0409283994003907,
            "MK": -0.0521084240793636,
            "PD": -0.0745381134111297,
            "WA": -0.0092263492839078,
            "SpaceHeating-MFH": -0.0409284,
            "SpaceHeating-EFH": -0.0495700,
            "Cooking_HotWater-HKO": 0,
        },
        "bH": {
            "BA": 0.463023693687715,
            "BD": 0.82307541850402,
            "BH": 0.693823369584483,
            "GA": 0.752645138542657,
            "GB": 0.930815856582958,
            "HA": 1.04488686764057,
            "KO": 0.862608575142234,
            "MF": 0.767292039450741,
            "MK": 0.864791873696473,
            "PD": 1.04630053886108,
            "WA": 0.45957571089625,
            "SpaceHeating-MFH": 0.7672920,
            "SpaceHeating-EFH": 0.8401015,
            "Cooking_HotWater-HKO": 0,
        },
        "mW": {
            "BA": -0.000713418600565782,
            "BD": -0.00192725690584626,
            "BH": -0.00128490078017325,
            "GA": -0.000908768552979623,
            "GB": -0.00139668882761774,
            "HA": -0.000829544720239446,
            "KO": -0.000880838956026602,
            "MF": -0.00223202741619469,
            "MK": -0.00143692105046127,
            "PD": -0.000367207932817838,
            "WA": -0.000967642449895133,
            "SpaceHeating-MFH": -0.0022320,
            "SpaceHeating-EFH": -0.0022090,
            "Cooking_HotWater-HKO": 0,
        },
        "bW": {
            "BA": 0.386744669887959,
            "BD": 0.107704598925155,
            "BH": 0.202973165694549,
            "GA": 0.191664070308203,
            "GB": 0.0850398799492811,
            "HA": 0.0461794912976014,
            "KO": 0.0964014193937084,
            "MF": 0.119920720218609,
            "MK": 0.0637601910393071,
            "PD": 0.0621882262236128,
            "WA": 0.396429075178636,
            "SpaceHeating-MFH": 0.1199207,
        ...
        "SO": {
            "BA": 0.9565000000000001,
            "BD": 0.9196,
            "BH": 0.9587000000000012,
            "GA": 1.0106000000000002,
            "GB": 0.9352999999999998,
            "HA": 0.8935000000000004,
            "KO": 0.9434999999999993,
            "MF": 0.9434999999999993,
            "MK": 0.9033999999999995,
            "PD": 0.8524999999999991,
            "WA": 0.4638,
            "SpaceHeating-MFH": 1.0,
            "SpaceHeating-EFH": 1.0,
            "Cooking_HotWater-HKO": 1.0,
...
def dict_wz(keys="WZ", values="Minititel", raw=False):
    ...
    """
    df = pd.read_excel(
        data_in("dimensionless", "WZ_2008.xlsx"), sheet_name="Nur_WZ", index_col=None
    )
