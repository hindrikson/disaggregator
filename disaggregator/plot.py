...
from .data import database_shapes_gisco, transpose_spatiotemporal
import os
...
def project_to_km2(df):
    DE_projected = df.to_crs("EPSG:3035")  # European projected CRS
    df["fl_km2"] = DE_projected.geometry.area / 1_000_000
    return df
...
def set_ax_format(
    ...
    if axtitle is not None:
        ...
        if axtitlesize is None:
            ax.set_title(axtitle, size=plt.rcParams["font.size"] + 2, weight=weight)
        else:
            ax.set_title(
                axtitle, size=plt.rcParams["font.size"] + axtitlesize, weight=weight
            )
    # xlabel
    ...
    if y_axis_numeric:
        (ax.get_yaxis().set_major_formatter(mticker.StrMethodFormatter("{x:,g}")))
    if minorticks == "off":
        ax.minorticks_off()
    elif minorticks == "on":
        ax.minorticks_on()
...
def heatmap_timeseries(df, **kwargs):
    ...
    if not isinstance(df, pd.DataFrame):
        raise ValueError("Please pass a DateTimeIndex'ed pd.DataFrame or pd.Series.")
    ...
    fig, ax = plt.subplots(
        figsize=figsize, nrows=nrows, ncols=1, sharex=True, squeeze=False
    )
    for col, ser in df.items():
        dfs = pd.DataFrame(np.array(ser).reshape(days, hours)).T
        cax = ax[i, j].imshow(
            dfs, interpolation="nearest", cmap=cmap, vmin=vmin, vmax=vmax
        )
...
def check_matching_indices(df, shape_df):
    df_indices = set(df.index)
    shapefile_indices = set(shape_df.index)

    # Symmetric difference: all non-common indices
    non_common_indices = df_indices ^ shapefile_indices

    # Specifically in df but not in shapefile
    missing_in_shapefile = df_indices - shapefile_indices

    # Specifically in shapefile but not in df
    missing_in_df = shapefile_indices - df_indices

    if non_common_indices:
        logger.warning(f"Indices not common to both dataframes: {non_common_indices}")
    else:
        logger.info("All indices match between both dataframes.")

    if missing_in_shapefile:
        logger.warning(f"Indices in df but NOT in shapefile: {missing_in_shapefile}")
        logger.warning(f"Indices in df but NOT in shapefile: {missing_in_shapefile}")
...
def choropleth_map(
    ...
    * Choropleth map = A map with countries colored according to a value.
    ...
    highlight_cities = kwargs.get("highlight_cities", cfg.get("highlight_cities", []))
    ...
    if shape_source_api:
        ...
        DE = database_shapes_gisco()
    else:
        # Create data directory
        data_dir = "data"
        os.makedirs(data_dir, exist_ok=True)
        base_year = cfg["base_year"]
        # Use correct dataset names from geodatasets
        # Define URLs and filenames based on year
        if base_year >= 2016:
            filename = os.path.join(data_dir, "NUTS_RG_01M_2016_4326_LEVL_3.shp")
            url = "https://gisco-services.ec.europa.eu/distribution/v2/nuts/shp/NUTS_RG_01M_2016_4326_LEVL_3.shp.zip"
            print(f"Using NUTS 2016 data for base year {base_year}")
        else:
            filename = os.path.join(data_dir, "NUTS_RG_01M_2013_4326_LEVL_3.shp")
            url = "https://gisco-services.ec.europa.eu/distribution/v2/nuts/shp/NUTS_RG_01M_2013_4326_LEVL_3.shp.zip"
            print(f"Using NUTS 2013 data for base year {base_year}")

        # Download if file doesn't exist
        if not os.path.exists(filename):
            print("NUTS shapefile not found locally. Downloading...")
            try:
                ...
                print(f"Error downloading NUTS data: {e}")
                raise
        else:
            print("Using existing NUTS shapefile.")
            print(filename)

        # Read the shapefile
        try:
            DE = gpd.read_file(filename)
            print(f"Loaded shapefile with {len(DE)} features")

            # Filter for Germany
            if "CNTR_CODE" in DE.columns:
                DE = DE[DE["CNTR_CODE"] == "DE"]
                print(f"Filtered to {len(DE)} German NUTS regions")

            DE = DE.set_index("NUTS_ID")
            print("Set NUTS_ID as index")
    ...
    # Handle the division ONCE, before processing units
    if relative:  # convert to unit per km²
        DF = project_to_km2(DF)
        for col in cols:
            DF[col] = DF[col] / DF.fl_km2
    ...
    if colorbar_each_subplot:
        ...
        if isinstance(interval, str) or (interval is None):
            intervals = [(DF[cols].min().min(), DF[cols].max().max()) for c in cols]
        else:
            intervals = [interval for c in cols]
    ...
    if nrows == 0 or ncols == 0:
        if nrows != ncols or ncols < 0 or nrows < 0:
            logger.warn(
                "When passing `nrows` and `ncols` both (!) must be "
                "passed as int and be greater zero. Gathering these "
                "values now based on the given pd.DataFrame."
            )
        nrows, ncols, rem = gather_nrows_ncols(len(cols), orientation=orientation)
    ...
    for a, col in enumerate(cols):
        ...
        (
            DF.dropna(subset=[col], axis=0).plot(
                ax=ax[i, j],
                column=col,
                cmap=cmap[a],
                edgecolor=edgecolor,
        ...
        if add_percentages:
            ratio = "{:.1f}".format(df[col].sum() / df.sum().sum() * 100.0)
            ratio = "({}%)".format(ratio)
        ...
        for idx, row in DF.iterrows():
            ...
            for a, ann in enumerate(annotate):
                ...
                if ann in ["value", "values"]:
                    if annotate_zeros:
                        s += "" if np.isnan(row[col]) else "{:.0f}".format(row[col])
                    else:
                        s += (
                            ""
                            if (np.isnan(row[col]) or row[col] == 0.0)
                        ...
                        if annotate_zeros:
                            s += (
                                ...
                                if np.isnan(df.loc[idx, col])
                                    ...
                                    df.loc[idx, col] / float(df[col].sum())
                                )
                            )
                        else:
                            s += (
    ...
    if not hide_colorbar:
        if colorbar_each_subplot:
            for a, axes in enumerate(ax.ravel().tolist()):
                ...
                sm = ScaMap(
                    cmap=cmap[a],
                    norm=plt.Normalize(vmin=intervals[a][0], vmax=intervals[a][1]),
                )
                sm._A = []
            ...
            sm = ScaMap(
                cmap=cmap[0],
                norm=plt.Normalize(vmin=intervals[0][0], vmax=intervals[0][1]),
            )
            sm._A = []
...
def get_cities(types=["city", "town"]):
    CI = (
        gpd.read_file(
            data_in("regional", "shapes", "germany_places", "places.shp"),
            encoding="utf-8",
        )
    ...
    CI["coords"] = CI.geometry.apply(lambda x: x.representative_point().coords[:][0])
    return CI


def add_license_to_figure(
    fig, license="CC BY 4.0", geotag=True, into_ax=True, **kwargs
):
    """
    ...
    if into_ax:
        ...
        fig.text(x, y, s, fontsize=6, color="gray", ha="left", va="bottom", alpha=0.5)
...
def cmap_handler(cmap, **kwargs):
    ...
    if (
        isinstance(cmap, Colormap)
        or isinstance(cmap, ListedColormap)
        ...
        ser_cmap = (
            pd.read_excel(f_cmaps, sheet_name=sheet_name, index_col="order")
            .dropna(axis=0, how="all")
            .loc[lambda x: x["cmap"] == cmap]
        )["hex"]
        if len(ser_cmap) == 0:
            raise ValueError(f"'{cmap}' not contained in file \n{f_cmaps}")

        nodes = np.linspace(0, 1, len(ser_cmap))
        return LinearSegmentedColormap.from_list(cmap, list(zip(nodes, ser_cmap)))

    elif isinstance(cmap, list):
        logger.warning(
            "list-handling is not yet supported by `cmap_handler`, returning as list!"
        )
        return cmap
    else:
...
def handle_plot_mode(s, **kwargs):
    ...
    if s == "screen" or s == "fullhd":  # Close to FullHD Resolution
        ...
        raise ValueError(
            "Wrong plot mode given! Must be any of ['screen', "
