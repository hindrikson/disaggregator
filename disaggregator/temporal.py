...
from .config import (
    ...
    slp_branch_cts_gas as slp_wz_g,
    data_in,
    slp_branch_cts_power as slp_wz_p,
    dict_region_code,
    slp_household_gas,
    blp_wz_list,
    check_matching_indices,
)
from .data import (
    elc_consumption_HH,
    households_per_size,
    ...
    living_space,
    h_value,
    zve_percentages_applications,
    zve_percentages_baseload,
    zve_application_profiles,
    database_shapes_gisco,
    CTS_power_slp_generator,
    t_allo,
    shift_load_profile_generator,
    gas_slp_weekday_params,
    percentage_EFH_MFH,
    regional_branch_load_profiles,
)
from .spatial import (
    disagg_CTS_industry,
...
def disagg_temporal(spat, temp, time_indexed=False, **kwargs):
    ...
    if not isinstance(spat, pd.Series):
        raise ValueError(
            "`spat` must be either a pd.Series or a one-level-indexed (!) pd.DataFrame."
        )
    ...
    if isinstance(temp, pd.Series):
        ...
        if time_indexed:
            return pd.DataFrame(
                np.outer(temp, spat), index=temp.index, columns=spat.index
            )
        else:
            return pd.DataFrame(
                np.outer(spat, temp), index=spat.index, columns=temp.index
            )
        ...
        raise ValueError(
            "`temp` must be either a pd.Series or a one-level-indexed (!) pd.DataFrame."
        )


def make_zve_load_profiles(
    return_profile_by_typeday=False, return_profile_by_application=False, **kwargs
):
    """
    ...
    if 6 in df_HH and df_HH[6] is not None:
        df_HH[5] = df_HH[5] + df_HH[6]
    if 6 in df_HH.columns:
    ...
    # DE = database_shapes() # NOTE: original line
    DE = database_shapes_gisco()  # updated line
    ...
    idx = pd.date_range(
        start="{}-01-01".format(year), end="{}-12-31 23:00".format(year), freq="1h"
    )
    if reg is not None:
        DE = DE.loc[reg].to_frame().T
    DF = pd.DataFrame(index=idx, columns=DE.index)

    check_matching_indices(df_elc_HH_share, DE)
    # NOTE: There is a mismatch so I filter DE for the indices in df_elc_HH_share
    excluded_indices = DE.index.difference(df_elc_HH_share.index)
    DE = DE.loc[DE.index.intersection(df_elc_HH_share.index)]
    logger.warning(
        "The following regions are excluded from DE due to missing household data: {}".format(
            excluded_indices.tolist()
        )
    )
    for region, row in tqdm(DE.iterrows(), total=len(DE), desc="Creating ZVE-profiles"):
        try:
            ...
            time_idx = pd.date_range(
                start="2012-01-02 00:00:00", end="2012-01-02 23:45:00", freq="15Min"
            )
            LP_15 = xr.DataArray(
                np.zeros((nTsLP, n_app_activity, n_ts, n_HH)),
                dims=["Time", "Application", "TimeSlice", "HH_size"],
                coords=[time_idx, df_perc_app.index[0:9], time_slices, df_HH.columns],
            )
                ...
                for id_day, df_day in df_HH_size.groupby("Day"):
                    for id_season, df_season in df_day.groupby("Season"):
                        ...
                        for id_app, df_app in df_season.groupby("Application"):
                            LP_15[:, id_app - 1, id_ts, HH_size - 1] = df_app.iloc[
                                0, 4:100
                            ]

            LP_Fin = xr.DataArray(
                np.zeros((nTsLP, n_app_all, n_ts), dtype=float),
                dims=["Time", "Application", "TimeSlice"],
                coords=[time_idx, l_app_all, time_slices],
            )

            LP_HHGr = xr.DataArray(
                np.zeros((nTsLP, n_ts, n_HH), dtype=float),
            ...
            for i_HH, HH_size in enumerate(df_elc_HH.columns):
                ...
                for i_ts, ts in enumerate(time_slices):
                    ...
                    year_sum += (
                        LP_15[:, 0, i_ts, i_HH].sum(axis=0).item()
                ...
                norm_factor_0 = (nTsLP / 24.0) * 1000000.0 / 366.0
            ...
            df_erg.columns = pd.MultiIndex.from_product(
                [["WD", "SA", "SU"], ["Win", "Tra", "Sum"]]
            )
            ...
            logger.error("Error processing region {}: {}".format(region, e))
...
def getSunsetSunrise(doy, lat, lon, UTC_diff):
    ...
    # time difference
    time_diff_arg = (math.sin(sunset_h) - math.sin(B) * math.sin(declination_sun)) / (
        math.cos(B) * math.cos(declination_sun)
    )
    time_diff_arg = min(time_diff_arg, 1.0)
    time_diff = 12 * math.acos(time_diff_arg) / math.pi
    time_equation = -0.171 * math.sin(0.0337 * doy + 0.465) - 0.1299 * math.sin(
        0.01787 * doy - 0.168
    )
...
def probability_light_needed(lat, lon, nTsLP=96):
    ...
    p_night = xr.DataArray(
        np.zeros((nTsLP, len(seasons)), dtype=np.float32),
        dims=["Time", "Season"],
    ...
    for season_id, season in enumerate(seasons):
        ...
        add = 1.0 / float(iadd)  # Percentage of one day in that season
        TsH = nTsLP / 24.0  # Number of time steps per Hour
        for m in season_months[season]:
            ...
            for d in range(0, month_lengths[m - 1]):
                ...
                UTC_diff = float(
                    timezone
                    + min(max(doy - SummerTimeBegin, 0), 1)
...
def disagg_temporal_power_CTS_blp(detailed=False, use_nuts3code=False, **kwargs):
    """
    ...
    blp_ghd = [x for x in slp_wz_p().keys() if x in blp_wz_list()]
    slp_ghd = [x for x in slp_wz_p().keys() if x not in blp_wz_list()]

    ec_slp = (
        sv_yearly.reindex(index=slp_ghd)
        .transpose()
        .assign(BL=lambda x: [bl_dict().get(int(i[:-3])) for i in x.index.astype(str)])
    )
    ...
    for state in bl_dict().values():
        ...
        sv_lk_wz = (
            ec_slp.loc[lambda x: x["BL"] == state]
            .drop(columns=["BL"])
            .fillna(value=0)
        ...
        for slp in sv_lk_wz["SLP"].unique():
            ...
            sv_lk = (
                sv_lk.assign(
                    LK_WZ=lambda x: x.level_1.astype(str) + "_" + x.level_0.astype(str)
                )
                .set_index("LK_WZ")
            ...
            if detailed:  # Calculate load profile for each LK and WZ
                ...
                lp_lk = pd.DataFrame(
                    np.multiply(slp_bl[[slp]].values, sv_dtl_df.values),
                    index=slp_bl.index,
    ...
    if detailed:
        DF2 = pd.DataFrame()
        logger.info(
            "Start downloading BLP from Database now. This may take a"
        ...
        for branch in blp_ghd:
            if branch in [
                ...
                if branch_model_64 is None:
                    branch_model_64 = regional_branch_load_profiles(wz=64, year=year)
                demand_branch = np.multiply(
                    branch_model_64, ec_blp.loc[ec_blp.index == branch]
                )
            elif branch in [42, 43]:
                # these branches use the same blp, which is wz=41
                logger.info("Working on WZ" + str(branch))
                if branch_model_41 is None:
                    branch_model_41 = regional_branch_load_profiles(wz=41, year=year)
                demand_branch = np.multiply(
                    branch_model_41, ec_blp.loc[ec_blp.index == branch]
                )
            else:
                ...
                demand_branch = np.multiply(
                    branch_model, ec_blp.loc[ec_blp.index == branch]
                )
            # concat demand_branch to DF2, DF2 has same index as DF
            DF2 = pd.concat(
                [
                    DF2,
                    (demand_branch.T.assign(WZ=branch).set_index("WZ", append=True).T),
                ],
                axis=1,
            )
    ...
    assert np.isclose(total_sum, disagg_sum, rtol=1.0e-3), msg.format(
        total_sum, disagg_sum
    )

    if use_nuts3code:
        total_demand = total_demand.rename(
            columns=dict_region_code(level="lk", keys="ags_lk", values="natcode_nuts3"),
...
def disagg_temporal_power_housholds_slp(
    use_nuts3code=False, by="population", weight_by_income=False, **kwargs
):
    """
    ...
    sv_yearly = (
        (
            disagg_households_power(by=by, weight_by_income=weight_by_income, year=year)
            * 1e3
        )
        .rename(index=dict_region_code(keys="natcode_nuts3", values="ags_lk"))
        .to_frame()
        .assign(BL=lambda x: [bl_dict().get(int(i[:-3])) for i in x.index.astype(str)])
    )
...
def disagg_daily_gas_slp_cts(state, temperatur_df, **kwargs):
    ...
    gv_lk = gv_lk.assign(
        BL=[bl_dict().get(int(x[:-3])) for x in gv_lk.index.astype(str)]
    )
...
def disagg_daily_gas_slp_households(state, temperatur_df, how="top-down", **kwargs):
    """
    Returns daily demand of gas with a given yearly demand in MWh
    per district and SLP.
    ...
    gv_lk = pd.DataFrame({"EFH": df_gas_EFH, "MFH": df_gas_MFH})
    gv_lk_return = gv_lk.copy()
    gv_lk = gv_lk.rename(
        index=dict_region_code(keys="natcode_nuts3", values="ags_lk")
    ).assign(BL=lambda x: [bl_dict().get(int(i[:-3])) for i in x.index.astype(str)])
    ...
    df = df.iloc[days:]
    df.columns = pd.MultiIndex.from_tuples(
        [(int(x), str(y)) for x, y in df.columns.str.split("_")]
    )
    return [df, gv_lk_return]
...
def disagg_temporal_gas_CTS(detailed=False, use_nuts3code=False, **kwargs):
    ...
    df = pd.DataFrame(
        0,
        columns=temperatur_df.columns,
        index=pd.date_range((str(year) + "-01-01"), periods=hours, freq="H"),
    )
    for state in bl_dict().values():
        logger.info("Working on state: {}.".format(state))
        ...
        for lk in list_lk:
            ...
            tw_df_lk = tw_df_lk.resample("h").ffill().infer_objects(copy=False)
            ...
            templ_cal = pd.concat([temp_cal, last_hour])
            # temp_cal = temp_cal.resample("H").pad()
            temp_cal = templ_cal.resample("h").ffill().infer_objects(copy=False)
            temp_cal = temp_cal[:-1]
...
def disagg_temporal_gas_households(use_nuts3code=False, how="top-down", **kwargs):
    """
    ...
    for state in bl_dict().values():
        logger.info("Working on state: {}.".format(state))
        tw_df, gv_lk = disagg_daily_gas_slp_households(
            state, temperatur_df, year=year, how=how
        )
        gv_lk = gv_lk.rename(
            index=dict_region_code(keys="natcode_nuts3", values="ags_lk")
        ).assign(BL=lambda x: [bl_dict().get(int(i[:-3])) for i in x.index.astype(str)])
        ...
        # Collect all lk data for this state
        state_lk_data = {}

        for lk in list_lk:
            lk_df_data = {}  # Store individual columns for this lk
            ...
            temp_cal = temp_cal.resample("h").ffill().infer_objects(copy=False)
            temp_cal = temp_cal[:-1]
            temp_cal["Stunde"] = pd.DatetimeIndex(temp_cal.index).time
            temp_cal = temp_cal.set_index(["Tagestyp", lk, "Stunde"])
            ...
            for slp in [slp_household_gas()[x] for x in tw_df_lk.columns.values]:
                ...
                temp_cal["Prozent"] = [slp_profil[x] for x in temp_cal.index]
                for wz in [
                    k for k, v in slp_household_gas().items() if v.startswith(slp)
                ]:
                    column_name = str(lk) + "_" + str(wz)
                    column_data = tw_df_lk[wz].values * temp_cal["Prozent"].values / 100
                    lk_df_data[column_name] = column_data

            # Store the sum for this lk
            if lk_df_data:  # Only if we have data
                lk_sum = sum(lk_df_data.values())
                state_lk_data[str(lk)] = lk_sum
                # Add individual columns to the collection
                state_lk_data.update(lk_df_data)

        # Convert state data to DataFrame and add to collection
        if state_lk_data:
            state_df = pd.DataFrame(
                state_lk_data,
                index=pd.date_range((str(year) + "-01-01"), periods=hours, freq="h"),
            )
            all_columns_data.append(state_df)

    # Create final DataFrame efficiently - FIXED SECTION
    if all_columns_data:
        # Concatenate all data at once instead of column-by-column assignment
        df = pd.concat(all_columns_data, axis=1)
    else:
        # Create empty DataFrame with proper index if no data
        df = pd.DataFrame(
            index=pd.date_range((str(year) + "-01-01"), periods=hours, freq="h")
        )

    # Select only the required columns if gv_lk exists
    if "gv_lk" in locals() and not gv_lk.empty:
        available_cols = [col for col in gv_lk.index.astype(str) if col in df.columns]
        if available_cols:
            df = df[available_cols]
            df.columns = df.columns.astype(int)

    if use_nuts3code and not df.empty:
        df = df.rename(
            columns=dict_region_code(level="lk", keys="ags_lk", values="natcode_nuts3")
        )
    return df
...
def disagg_temporal_industry_blp(
    source="power",
    detailed=False,
    use_nuts3code=False,
    low=0.4,
    ...
    **kwargs,
    ...
    if source != "power":
        raise ValueError(
            'blp can only used for electricity consumption.change source to "power".'
        )
    # Obtain yearly power consumption per WZ per LK
    ec_yearly = disagg_CTS_industry(
        "power", "industry", year=year, no_self_gen=no_self_gen
    )
    # splitting yearly demand in two groups. slp group are disaggregated with
    # shift load profiles. blp group are disaggregated with blp
    # (=profiles based on metered load data from the demandregio project)
    blp_i = [x for x in shift_profile_industry().keys() if x in blp_wz_list()]
    slp_i = [x for x in shift_profile_industry().keys() if x not in blp_wz_list()]
    ...
    for state in bl_dict().values():
        ...
        sv_lk_wz = (
            ...
            .infer_objects(copy=False)
            .transpose()
            .assign(SP=lambda x: [shift_profile_industry()[i] for i in x.index])
        )
        logger.info("... creating state-specific load-profiles")
        ...
        for sp in sv_lk_wz["SP"].unique():
            ...
            if detailed:  # Calculate load profile for each LK and WZ
                lp_lk_wz = pd.DataFrame(
                    np.multiply(sp_bl[[sp]].values, sv_lk.values),
                    index=sp_bl.index,
                    columns=sv_lk.columns,
                )
            else:  # Calculate load profile for each LK
                lp_lk = pd.DataFrame(
                    np.multiply(sp_bl[[sp]].values, sv_dtl_df.values),
                    index=sp_bl.index,
                    columns=sv_dtl_df.columns,
                )
            # Merge intermediate results
    ...
    if detailed:
        ...
        DF2 = pd.DataFrame(
            0,
            index=pd.date_range(
                freq="15Min", start="01/01/" + str(year), periods=len(DF)
            ),
            columns=ec_blp.columns,
        )
        logger.info(
...
def disagg_temporal_industry(
    source, detailed=False, use_nuts3code=False, low=0.4, no_self_gen=False, **kwargs
):
    """
    Disagreggate spatial data of industrie's power or gas demand temporally.
    ...
    # Obtain yearly power consumption per WZ per LK
    ec_yearly = (
        disagg_CTS_industry(source, "industry", no_self_gen=no_self_gen, year=year)
        .transpose()
        .assign(BL=lambda x: [bl_dict().get(int(i[:-3])) for i in x.index.astype(str)])
    )
    ...
    for state in bl_dict().values():
        logger.info("Working on state: {}.".format(state))
        sv_lk_wz = (
            ec_yearly.loc[lambda x: x["BL"] == state]
            .drop(columns=["BL"])
            .fillna(value=0)
            .infer_objects(copy=False)
            .transpose()
