...
def disagg_households_power(by, weight_by_income=False, scale_by_pop=False, **kwargs):
    """
    ...
    if by == "households":
        ...
        df = households_per_size(original=scale_by_pop, year=year) * power_per_HH
    elif by == "population":
...
def disagg_households_gas(
    how="top-down", weight_by_income=False, original=False, **kwargs
):
    """
    ...
    # df_ls_gas = living_space(aggregate=True, internal_id=[None, None, 11, 1]).sum(
    #     axis=1
    # )
    df_ls_gas = living_space(
        aggregate=True,
        internal_id={
            "internal_id_1": None,
            "internal_id_2": None,
    ...
    if how == "top-down":
        ...
        # Calculate
        df = pd.DataFrame(index=df_ls_gas.index).assign(
            Cooking=d_keys_cook * gas_nuts0["Cooking"],
        ...
        df1 = (
            heat_demand_buildings(
                table_id=56, year=2018, internal_id=[None, None, 1, 1]
            )
        ...
        df2 = (
            heat_demand_buildings(
                table_id=56, year=2018, internal_id=[None, None, 1, 2]
            )
            .replace(dict(vintage_class=new_dem_vintages))
            .loc[lambda x: x.vintage_class == "A_<1948"]
        )
        ...
        piv_dem = (
            df_heat_dem.pivot_table(
                values="value", index="nuts3", columns="vintage_class", aggfunc="mean"
            )
            / 1e3
        )  # kWh -> MWh
        piv_m2 = df_ls_gas.pivot_table(
            values="value", index="nuts3", columns="vintage_class", aggfunc="sum"
        )
        df_erg = piv_dem.multiply(piv_m2) / 0.99  # eta-boiler assumption
        df_spaceheat = df_erg.sum(axis=1)
        # Calculate
        df = pd.DataFrame(index=df_spaceheat.index).assign(
            Cooking=d_keys_cook * gas_nuts0["Cooking"],
            HotWater=d_keys_hotwater * gas_nuts0["HotWater"],
        ...
        logger.warning(
            "This feature is currently experimental and should not "
        ...
        for idx, row in df_stove.iterrows():
            df_heat_cook.update(
                df_heat_cook.filter(axis=0, regex=idx)
                * row.stoves_percentage_gas
                / row.stoves_efficiency_gas
            )

        logger.info("2. Hot water (decentralised) based on household sizes in [MWh/a]")
        df_WW_shares = hotwater_shares()
        df_WW = pd.DataFrame(df_heat_dem["HotWater"])
        for idx, row in df_WW_shares.iterrows():
            df_WW.update(
                df_WW.filter(axis=0, regex=idx) * row.share_decentralised_gas / 0.95
            )  # efficiency assumption gas boilers

        logger.info(
            "3. Space heating + hot water (centralised) based on "
            "living space in [MWh/a]"
        )

        df_hc_only = heat_consumption_HH(by="buildings").T.loc[:, "SpaceHeatingOnly"]
        df_hc_HW = heat_consumption_HH(by="buildings").T.loc[
            :, "SpaceHeatingPlusHotWater"
        ]
        df_spaceheat = df_ls_gas.multiply(df_hc_only)
        df_spaceheat_HW = df_ls_gas.multiply(df_hc_HW)
        for idx, row in df_WW_shares.iterrows():
            df_spaceheat.update(
                df_spaceheat.filter(axis=0, regex=idx) * (1.0 - row.share_centralised)
            )
            df_spaceheat_HW.update(
                df_spaceheat_HW.filter(axis=0, regex=idx) * row.share_centralised
            )

        logger.info("4. Merging results")
        df = pd.DataFrame(index=df_heat_dem.index).assign(
            Cooking=df_heat_cook.sum(axis=1),
...
def disagg_households_heat(by, weight_by_income=False, **kwargs):
    ...
    if by not in ["households", "buildings"]:
        raise ValueError(
            "The heating demand of households depends mainly on "
...
def disagg_CTS_industry(
    source, sector, use_nuts3code=False, no_self_gen=False, **kwargs
):
    """
    ...
    df = pd.DataFrame(
        data=(
            employees_per_branch(year=year).loc[spez_vb.index].values * spez_vb.values
        ),
        index=spez_vb.index,
        columns=spez_vb.columns,
    )
    df = df.multiply(
        efficiency_enhancement(source, year=year).transpose().loc[df.index], axis=0
    )
    if use_nuts3code:
        df = df.rename(columns=dict_region_code(keys="ags_lk", values="natcode_nuts3"))
    return df
